...
| `publish_path`             | `PODMAN_PUBLISH_PATH`          | `podman`                                                 | Where `podman:setup`/`podman:install` write every rendered `.quadlets` file, install or not — see [Setting up without PHP on the host](host-setup.md) |
