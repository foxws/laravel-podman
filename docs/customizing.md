...
| `working_path`             | `PODMAN_WORKING_PATH`          | Laravel's `base_path()`                                  | Real host path baked into the `{{workingPath}}`/`{{runtimePath}}` placeholders only — doesn't affect where files are actually read/written, see [Setting up without PHP on the host](host-setup.md). `podman:generate` also accepts `--working-path=` to override it for a single run |
