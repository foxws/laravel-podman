...
| `publish_path`             | `PODMAN_PUBLISH_PATH`          | `storage/app/podman`                                     | Where `podman:setup`/`podman:install` write rendered `.quadlets` files when `--publish` is passed, or `podman` isn't found — see [Setting up without PHP on the host](../README.md#setting-up-without-php-on-the-host) |
